...
def _run_case(
    ...
    *,
    ...
    result = planning.run_stage6_live_case(
        ...
        or {"selected_option_id": "option:case"},
        agent="codex",
        model=None,
...
def test_server_binds_exact_stage5_causal_coverage_without_copy_drift_correction(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    stage5_coverage = {
        "mechanism_addressed": "Bound image accumulation exhausts the Docker store.",
        "research_binding": {
            "mechanism_symbols": ["runner_core.execution_backend._run"],
            "causal_intervention": "Remove eligible identities before the next build.",
        },
        "residual_recurrence_paths": ["Protected images can consume the remaining budget."],
    }
    planner_plan = _plan()
    planner_plan["causal_coverage"] = {
        **stage5_coverage,
        "research_binding": {
            **stage5_coverage["research_binding"],
            # A one-word nested paraphrase must not trigger a same-author rewrite: the
            # planner does not own this already-selected Stage 5 contract.
            "causal_intervention": "Remove eligible images before the next build.",
        },
    }
    result, transport = _run_case(
        tmp_path,
        monkeypatch,
        [{"response": json.dumps([planner_plan])}],
        selection_decision={
            "selected_option_id": "option:case",
            "selected_option": {
                "option_id": "option:case",
                "causal_coverage": stage5_coverage,
            },
        },
    )
    ...
    assert result["plans"][0]["causal_coverage"] == stage5_coverage
    assert (
        result["plans"][0]["causal_coverage"]["research_binding"]["causal_intervention"]
        == "Remove eligible identities before the next build."
    )


def test_server_binds_exact_stage5_scope_evidence_without_copy_drift_correction(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    stage5_scope = {
        "scope_level": "multiple_independent_paths",
        "independent_consumers_or_failure_paths": [
            {
                "name": "Batch maintenance-image preparation",
                "evidence_refs": ["experiment:batch-capacity-baseline"],
            },
            {
                "name": "Direct maintenance-image resolution",
                "evidence_refs": ["experiment:direct-capacity-baseline"],
            },
        ],
        "shared_contract_justification": "Both paths consume the same prepared identity.",
    }
    planner_plan = _plan()
    planner_plan["scope_evidence"] = {
        **stage5_scope,
        "independent_consumers_or_failure_paths": [
            {
                **stage5_scope["independent_consumers_or_failure_paths"][0],
                # One nested word differs from the immutable selected-option contract.
                "name": "Batch maintenance-image setup",
            },
            stage5_scope["independent_consumers_or_failure_paths"][1],
        ],
    }
    result, transport = _run_case(
        tmp_path,
    ...
    assert len(transport.calls) == 1
    assert result["metrics"]["correction_turn_count"] == 0
    assert result["plans"][0]["scope_evidence"] == stage5_scope
    assert (
        result["plans"][0]["scope_evidence"][
            "independent_consumers_or_failure_paths"
        ][0]["name"]
        == "Batch maintenance-image preparation"
    )


def test_stage5_causal_coverage_binding_is_an_exact_deep_copy() -> None:
    stage5_coverage = {
        "mechanism_addressed": "A bounded resource accumulates.",
        "research_binding": {
            "mechanism_symbols": ["module.symbol"],
            "causal_intervention": "Bound the resource by identity.",
        },
    }
    selection = {
        "selected_option": {
    ...
    bound = planning._bind_stage5_selected_option_contracts(
        {"change_plan_id": "plan:case:1", "causal_coverage": {"drift": True}},
        selection_decision=selection,
    )

    assert bound["causal_coverage"] == stage5_coverage
    assert bound["causal_coverage"] is not stage5_coverage
    assert bound["causal_coverage"]["research_binding"] is not stage5_coverage[
        "research_binding"
    ]
    bound["causal_coverage"]["research_binding"]["mechanism_symbols"].append(
        "module.other"
    )
    assert stage5_coverage["research_binding"]["mechanism_symbols"] == ["module.symbol"]


def test_stage5_scope_evidence_binding_is_an_exact_deep_copy() -> None:
    stage5_scope = {
        "scope_level": "multiple_independent_paths",
        "independent_consumers_or_failure_paths": [
            {
                "name": "Batch preparation",
                "evidence_refs": ["experiment:batch"],
            }
        ],
    }
    selection = {
    ...
    bound = planning._bind_stage5_selected_option_contracts(
        {"change_plan_id": "plan:case:1", "scope_evidence": {"drift": True}},
        selection_decision=selection,
    )

    assert bound["scope_evidence"] == stage5_scope
    assert bound["scope_evidence"] is not stage5_scope
    assert bound["scope_evidence"]["independent_consumers_or_failure_paths"] is not (
        stage5_scope["independent_consumers_or_failure_paths"]
    )
    bound["scope_evidence"]["independent_consumers_or_failure_paths"][0][
        "evidence_refs"
    ].append("experiment:other")
    assert stage5_scope["independent_consumers_or_failure_paths"][0]["evidence_refs"] == [
        "experiment:batch"
    ]
...
def test_37_to_8_to_2_frontier_survives_costly_nonprogress_after_regression(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    errors_37 = [f"initial:error:{index}" for index in range(37)]
    errors_8 = [f"reduced:error:{index}" for index in range(8)]
    errors_2 = ["near_ready:error:a", "near_ready:error:b"]
    regression_errors = [
        "replacement:error:a",
        "replacement:error:b",
        "replacement:error:c",
    ...
    result, transport = _run_case(
        ...
        [
            {
                "response": json.dumps([_plan(errors=errors_37)]),
                "elapsed": 10.0,
            },
            {
                "response": json.dumps([_plan(errors=errors_8)]),
                "resume": _PLANNER_SESSION,
                "elapsed": 20.0,
            },
            {
                "response": json.dumps([_plan(errors=errors_2)]),
                "resume": _PLANNER_SESSION,
                "elapsed": 20.0,
            },
            {
                "response": json.dumps([_plan(errors=regression_errors)]),
                "resume": _PLANNER_SESSION,
                "elapsed": 5.0,
            },
            {
                "response": json.dumps(
                    [_plan(errors=[*regression_errors, "regression:error:d"])]
                ),
                "resume": _PLANNER_SESSION,
                # The legacy controller paused here because this nonadvancing repair turn
                # cost more than the original author turn, despite the retained 2-error best.
                "elapsed": 30.0,
            },
            {
                "response": json.dumps([_plan()]),
    ...
    assert [len(attempt["validation_errors"]) for attempt in history] == [
        37,
        8,
        2,
        3,
    ...
    assert [
        attempt["correction_progress"]["reason"] for attempt in history[1:]
    ] == [
    ...
    assert result["metrics"]["total_correction_cost_seconds"] == 80.0


def test_nonprogress_pause_restores_two_error_objective_best_after_regressions(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    best_response = json.dumps(
        [_plan(errors=["near_ready:error:a", "near_ready:error:b"])]
    )
    result, transport = _run_case(
        tmp_path,
        monkeypatch,
        [
            {
                "response": json.dumps(
                    [_plan(errors=[f"initial:error:{index}" for index in range(37)])]
                ),
                "elapsed": 1.0,
            },
            {
                "response": json.dumps(
                    [_plan(errors=[f"reduced:error:{index}" for index in range(8)])]
                ),
                "resume": _PLANNER_SESSION,
                "elapsed": 1.0,
            },
    ...
    assert len(transport.calls) == 6
    assert result["metrics"]["best_error_count"] == 2
    assert result["role_run"]["response_sha256"] == sha256(
        best_response.encode("utf-8")
    ).hexdigest()
...
def test_future_solution_execution_gap_returns_to_same_planner_for_a_plan(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    misplaced_return = {
        ...
        "evidence_gaps": [
            {
                "gap": (
                    "No runner-verified live command has executed the selected "
                    "post-change capacity-recovery outcome."
                ),
                "evidence_phase": "post_change_outcome_execution",
                "blocks": [
                    "The exact future live verification command and predicates."
                ],
                "evidence_needed": (
                    "Implement the selected solution and execute its live recovery probe."
                ),
                "evidence_refs": ["boundary:live-proof-not-yet-executed"],
            }
        ],
        "rationale": "The future command has not run yet.",
    }
    result, transport = _run_case(
        tmp_path,
        monkeypatch,
        [
            {"response": json.dumps([misplaced_return])},
            {
                "response": json.dumps([_plan()]),
                "resume": _PLANNER_SESSION,
            },
        ...
        research_dossier={
            "problem_id": "problem:case",
            "evidence_boundaries": ["boundary:live-proof-not-yet-executed"],
        },
    )
    ...
    assert len(transport.calls) == 2
    correction_prompt = str(transport.calls[1]["prompt"])
    assert "planner_research_gap_evidence_phase_invalid:0" in correction_prompt
    assert "SAME-AUTHOR PLANNER RESPONSE CORRECTION" in correction_prompt
