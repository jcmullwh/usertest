...
def parse_problem_mining_response_envelope(text: str) -> dict[str, Any]:
    """Extract the strict stage-1 response object from model output."""
    ...
    if not isinstance(parsed, dict):
        raise ValueError("problem_mining_response_envelope_required")
    if set(parsed) != {"problem_records", "atom_decisions"}:
        raise ValueError("problem_mining_response_envelope_fields_invalid")
    if not isinstance(parsed.get("problem_records"), list) or not isinstance(
        parsed.get("atom_decisions"), list
    ):
        raise ValueError("problem_mining_response_envelope_lists_required")
    return dict(parsed)
    ...
    with path.open("r", encoding="utf-8") as stream:
        for line_number, line in enumerate(stream, start=1):
            ...
            try:
                ...
                raise ValueError(
                    f"problem_mining_normalized_event_invalid:{line_number}"
                ) from exc
...
def build_live_miner_receipt(
    *,
    ...
    chunk_atom_ids = [
        atom_id
        for chunk in chunks
        for atom_id in _string_list(chunk.get("atom_ids"))
    ]
    ...
    origin_manifest = (
        dict(origin_manifest_raw) if isinstance(origin_manifest_raw, Mapping) else {}
    )
    materialization_errors = [
        dict(error)
        for error in origin_manifest.get("errors", [])
        if isinstance(error, Mapping)
        and _text(error.get("atom_id")) in set(assigned)
    ]
    ...
    for index, raw_record in enumerate(records):
        ...
        if preview_only:
            raise ValueError(
                f"problem_mining_preview_only_citation:{tag}:{problem_id}:"
                + ",".join(preview_only)
            )
    ...
    for problem_id, evidence_ids in record_evidence.items():
        for atom_id in evidence_ids:
            ...
            if (
                decision.get("disposition") != "supports_case"
                or problem_id not in _string_list(decision.get("problem_ids"))
            ):
                raise ValueError(
                    f"problem_mining_citation_without_support_decision:{tag}:"
                    f"{problem_id}:{atom_id}"
                )
    unread_assigned = sorted(set(assigned) - set(reads))
    if unread_assigned:
        raise ValueError(
            f"problem_mining_assigned_atom_not_read_in_full:{tag}:"
            + ",".join(unread_assigned)
        )
...
def build_failed_miner_receipt(
    *,
    ...
    return {
        ...
        "normalized_events_sha256": None,
        "read_attestations": [],
        "origin_attachment_evidence": workspace_manifest.get(
            "origin_attachment_evidence", {}
        ),
        "origin_attachment_read_attestations": [],
        "atom_decisions": decisions,
...
def finalize_problem_mining_evidence_receipt(
    *,
    ...
    live_jobs_verified = all(
        isinstance(miner, Mapping) and miner.get("status") == "verified"
        for miner in miners
    )
    receipt: dict[str, Any] = {
        ...
        "status": (
            "verified"
            if mode == "live" and live_jobs_verified
            else "partial_failed_jobs"
            if mode == "live"
            else "dry_run_not_exportable"
        ),
        "eligible_for_shadow_export": mode == "live" and live_jobs_verified,
        "eligible_atom_ids": eligible_ids,
...
def problem_mining_evidence_receipt_ref(
    *, receipt: Mapping[str, Any], receipt_path: Path
    ...
    return {
        ...
        "eligible_source_atom_count": len(
            _string_list(receipt.get("eligible_source_atom_ids"))
        ),
    }
    ...
    *,
    ...
    for raw_attestation in attachment_attestations:
        ...
        if (
            ...
            errors.append(
                f"problem_mining_origin_attachment_attestation_changed:{tag}:{rel_path}"
            )
            continue
    ...
    for origin_error in origin_errors:
        ...
        if atom_id in set(assigned) and (decisions_by_id.get(atom_id or "") or {}).get(
            "disposition"
        ) != "unresolved":
            errors.append(
                f"problem_mining_unavailable_attachment_decision_changed:{tag}:{atom_id}"
            )
...
def verify_problem_mining_evidence_receipt(
    *,
    ...
    path_raw = _text(artifacts.get("problem_mining_evidence_receipt")) or _text(
        ref.get("path")
    )
    if path_raw is None:
    ...
    if sorted(set(source_ids) | set(derived_ids)) != sorted(eligible_ids) or set(
        source_ids
    ) & set(derived_ids):
    ...
    for atom_id in eligible_ids:
        ...
        if disposition_errors:
            errors.append(
                f"problem_mining_atom_disposition_invalid:{atom_id}:"
    ...
    if sorted(assigned_ids) != sorted(eligible_ids) or len(assigned_ids) != len(
        set(assigned_ids)
    ):
    ...
    if sorted(decision_ids) != sorted(eligible_ids) or len(decision_ids) != len(
        set(decision_ids)
    ):
    ...
    for raw in decisions:
        ...
        if (
            ...
            or raw.get("disposition_receipt_sha256")
            != disposition_receipt.get("receipt_sha256")
            or raw.get("rationale") != disposition_receipt.get("rationale")
            or raw.get("revisit_when")
            != _text(atom.get("disposition_revisit_when"))
        ):
