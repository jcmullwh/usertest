...
def add_token_monitor_command(sub: argparse._SubParsersAction) -> None:
    ...
    token_monitor_delegation_ab_p = token_monitor_sub.add_parser(
        "delegation-ab",
        help="Compare delegation-disabled and delegation-enabled maintenance run evidence.",
    )
    token_monitor_delegation_ab_p.add_argument(
        "--disabled-run",
        dest="disabled_runs",
        action="append",
        required=True,
        type=Path,
        help="Run directory for a delegation-disabled/control maintenance ticket.",
    )
    token_monitor_delegation_ab_p.add_argument(
        "--enabled-run",
        dest="enabled_runs",
        ...
        required=True,
        type=Path,
        help="Run directory for a delegation-enabled/treatment maintenance ticket.",
    )
    token_monitor_delegation_ab_p.add_argument(
        "--codex-sessions-root",
        type=Path,
        help="Override Codex sessions root for token joins.",
    )
    token_monitor_delegation_ab_p.add_argument(
        "--output-dir",
        type=Path,
        help="Output directory for A/B evidence artifacts (defaults to current directory).",
    )
    token_monitor_delegation_ab_p.add_argument(
        "--no-write",
        action="store_true",
        help="Print metadata-only A/B analysis JSON without writing artifacts.",
    )
    token_monitor_delegation_ab_p.add_argument(
        "--repo-root",
        type=Path,
    ...
    token_monitor_delegation_ab_p.set_defaults(func=_cmd_token_monitor_delegation_ab)
...
def _cmd_token_monitor_delegation_ab(args: argparse.Namespace) -> int:
    """Execute token-monitor delegation-ab for paired maintenance runs."""
    repo_root = _resolve_repo_root(args.repo_root)
    disabled_runs = [
        _resolve_optional_path(repo_root, run_dir) or run_dir.resolve()
        for run_dir in args.disabled_runs
    ]
    enabled_runs = [
        _resolve_optional_path(repo_root, run_dir) or run_dir.resolve()
        for run_dir in args.enabled_runs
    ]
    sessions_root = (
        (
            _resolve_optional_path(repo_root, args.codex_sessions_root)
    ...
    if args.no_write:
        analysis = analyze_delegation_ab(
            disabled_run_dirs=disabled_runs,
            enabled_run_dirs=enabled_runs,
    ...
    print(str(output_dir / "delegation_ab_validation.json"))
    print(str(output_dir / "delegation_ab_validation.md"))
    return 0


__all__ = [
    "add_token_monitor_command",
    "_cmd_token_monitor_analyze",
    "_cmd_token_monitor_batch_context",
    "_cmd_token_monitor_delegation_ab",
]
