...
def test_drain_phase_replans_invalid_stage6_candidate_and_launches_unrelated_work(
    ...
    invalid_path = ready_dir / "badbadbadbadbadb.md"
    ...
    monkeypatch.setattr(
        ...
        lambda *, candidate: (
            replanned.append(candidate.fingerprint)
            or tmp_path / "1.5 - to_plan" / candidate.ticket_path.name
        ),
    )
    monkeypatch.setattr(
        "usertest_implement.batch_runner._claim_ticket",
    ...
    assert replanned == ["badbadbadbadbadb"]
    assert launched == ["valid.md"]
    ...
    assert failure["replanned_ticket_path"] == str(tmp_path / "1.5 - to_plan" / invalid_path.name)
