...
def _normalize_verification_commands_for_execution(
    ...
    *,
    ...
    has_scaffold_health = any(
        "scaffold.py" in s.command
        and "run" in s.command
        and ("lint" in s.command or "test" in s.command)
        for s in plan
    )
...
def _run_verification_commands(
    *,
    ...
    if _is_windows() and not command_prefix:
        bash_probe_needed = False
        for item in commands:
            raw_command = item.command if isinstance(item, VerificationCommandSpec) else item
            if not isinstance(raw_command, str):
                continue
            normalized_command = raw_command.strip()
            if not normalized_command:
                continue
            if not normalized_command.replace("\\", "/").lower().startswith("bash "):
                continue
            if _looks_like_verification_rejection_sentinel(normalized_command):
                continue
            if (
                normalize_command_for_shell(
                    normalized_command,
                ...
                bash_probe_needed = True
                break
        if bash_probe_needed:
    ...
    for idx, item in enumerate(commands, start=1):
        ...
        if (
            ...
            and verification_commands_need_python(
                (
                    VerificationCommandSpec(
                        command=cmd_after_host_normalization,
                        track=track,
                    ),
                )
            )
